...
class CDNFilter
{
	...
	*/
	function __construct($baseUrl, $cdnUrl, $directories, $excludedPhrases, $disableForAdmin, $baseUrlScheme)
	{
		...
		if (trim($directories) == '') 
		{
			$directories = CDN_DEFAULT_DIRECTORIES;
		}
		// Create the array
	...
	*/
    protected function rewriteUrl($asset) 
	{
		...
		if (strpos($foundUrl, 'wp-content') > 2 && strpos($foundUrl, 'https:') == false && strpos($foundUrl, 'http:') == false) {
			$foundUrl = $this->baseUrlScheme . ':' . $foundUrl;
		}

		$parseUrl = parse_url($foundUrl, PHP_URL_SCHEME);

		if(!empty($_GET['debug_cdnfilter'])) {
			return print_r($parseUrl,true);
		}
		...
		if (strpos($foundUrl, $this->cdnUrl) !== false) {
			// already replaced
			return $foundUrl;
		}

		if (strpos($foundUrl, $this->baseUrl) === false) {
			$foundUrl = $this->baseUrlScheme . ':' . $this->baseUrl . $foundUrl;
		}
