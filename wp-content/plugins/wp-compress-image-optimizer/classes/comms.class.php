...
class wps_ic_comms {
	...
	public static function get_stats() {
		...
		if ($range == 'current_month') {
			...
			$stats = $wpdb->get_results("SELECT COUNT(ID) as count, created, original, compressed, saved FROM " . $wpdb->prefix . "ic_stats WHERE created>='" . date('Y-m-d', $month_start) . "' AND created<='" . date('Y-m-d',
																																																																																																									$month_end) . "' GROUP BY attachment_ID ORDER BY created DESC");
	...
	public static function media_library_stats() {
		global $wpdb, $wps_ic;
		...
		$output['settings']['generate_adaptive']          = $settings['generate_adaptive'];
		$output['settings']['generate_webp']              = $settings['generate_webp'];
		$output['settings']['retina']                     = $settings['retina'];
		$output['settings']['live-cdn']                   = $settings['live-cdn'];
		$output['settings']['lazy']                       = $settings['lazy'];
		$output['settings']['backup-location']            = $settings['backup-location'];
		$output['settings']['autopilot']                  = $settings['autopilot'];
		$output['settings']['optimization']               = $settings['optimization'];
		$output['settings']['preserve_exif']              = $settings['preserve_exif'];
		$output['settings']['otto']                       = $settings['otto'];
		$output['settings']['hide_compress']              = $settings['hide_compress'];
		$output['settings']['css']                        = $settings['css'];
		$output['settings']['js']                         = $settings['js'];
		$output['settings']['js-minify']                  = $settings['js-minify'];
		$output['settings']['css-minify']                 = $settings['css-minify'];
		$output['settings']['resize_larger_images']       = $settings['resize_larger_images'];
		$output['settings']['resize_larger_images_width'] = $settings['resize_larger_images_width'];
		$output['settings']['version']                    = $wps_ic::$version;
		/*
		$output['attachments']                            = $count_attachments;
		$output['thumbnails']                             = $count_attachments * $total_thumbnails;
		$output['thumbnail_sizes']                        = $thumbnail_sizes;
		$output['compressed_attachments']                 = $compressed_attachments;*/
