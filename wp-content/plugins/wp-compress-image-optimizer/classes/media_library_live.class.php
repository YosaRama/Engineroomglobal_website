...
class wps_ic_media_library_live extends wps_ic {
	...
	public function compress_details($imageID) {
		...
		if ($compressing['status'] == 'compressed' || ! empty($stats)) {
			...
			if ($user_credits->bytes->local_leftover <= 0) {
				$output .= '<a class="wps-ic-compress-live-no-credits" data-attachment_id="' . $imageID . '">Compress</a>';
			} else {
	...
	public function find_compressed_images($queue = false) {
		...
		$images            = get_posts(array('post_type'      => 'attachment',
																				 'posts_per_page' => - 1,
																				 'meta_query'     => array(// meta query takes an array of arrays, watch out for this!
																					 array('key'     => 'ic_stats',
																								 'compare' => 'EXISTS'))));
