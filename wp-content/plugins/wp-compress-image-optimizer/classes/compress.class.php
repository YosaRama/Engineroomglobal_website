...
class wps_ic_compress {
  ...
  public function update_thumbnail($attachment_ID, $images_array) {
    ...
    foreach ($images_array as $image_size => $image_data) {
      ...
      if ($temp_file) {
        clearstatcache();
        if ( ! copy($temp_file, $current_thumbnail_path)) {
          #var_dump('File not copied ' . $current_thumbnail_path);
        } else {
          #var_dump('File copied ' . $current_thumbnail_path . ' from: ' .$image_data['uri'] . ' size: ' .filesize($temp_file));
        }

      } else {
        #var_dump('File not downloaded ' . $image_data['uri']);
      }
      ...
      if ( ! empty($image_data['webp'])) {
        ...
        if ($temp_file) {
          ...
          if ( ! copy($temp_file, $current_thumbnail_path)) {
            ...
            #var_dump($current_thumbnail_path);
          }

          $webps[] = $current_thumbnail_path;

        } else {
          #var_dump('File not downloaded ' . $image_data['webp']);
        }
      }
