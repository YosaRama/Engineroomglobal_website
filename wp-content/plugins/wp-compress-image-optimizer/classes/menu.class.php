...
class wps_ic_menu extends wps_ic {
	...
	public function __construct() {
		...
		if (is_admin()) {
			...
			if ( ! empty($option['hide_compress']) && $option['hide_compress'] == '1') {
				if (!empty($_GET['dbg']) && $_GET['dbg'] == 'menu') {
					die('menu hide compress');
				}
	...
	public function add_toolbar_items($admin_bar) {
		...
		if (current_user_can('manage_options')) {
			$admin_bar->add_menu(array(
														 ...
														 'href'  => '#',
														 'meta'  => array(
															 'title' => __('WP Compress'),
															 'html'  => '<span class="wp-compress-admin-bar-icon"></span>'
														 ),
													 ));
		}
		...
		if (current_user_can('manage_options')) {
			$admin_bar->add_menu(array(
														 'id'     => 'wp-compress-settings',
														 'parent' => 'wp-compress',
														 'title'  => 'Settings',
														 'href'   => admin_url('options-general.php?page=wpcompress'),
														 'meta'   => array(
															 'title'  => __('Settings'),
															 'target' => '_self',
	...
	public function plugin_list_link($links) {
		if ( ! empty(self::$connected['response_key'])) {
			$links = array_merge(array('<a href="' . admin_url('options-general.php?page=wpcompress') . '">' . 'Settings' . '</a>'), $links);
		} else {
			$links = array_merge(array('<a href="' . admin_url('options-general.php?page=wpcompress') . '">' . 'Get Started' . '</a>'), $links);
		}
	...
	public function menu_init() {
		...
		#add_menu_page('WP Compress', 'WP Compress', 'manage_options', $this::$slug, array($this, 'render_admin_page'));
	}
